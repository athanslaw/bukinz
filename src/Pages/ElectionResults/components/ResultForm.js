...
import { allAgents, allVotingLevels, electionTypes, getPollingUnitsByWardId, getWardsByLgaId, users } from "../../../lib/url";

const ResultForm = ({formFields, handleFormSubmit, politicalParties}) => {
    const levels = [
        {
            "code": "LGA",
            "name": "LGA",
    ...
    let party1Label = {id:1, name:'party_1', code:'party_1'}
    let party2Label = {id:1, name:'party_1', code:'party_2'}
    let party3Label = {id:1, name:'party_1', code:'party_3'}
    let party4Label = {id:1, name:'party_1', code:'party_4'}
    let party5Label = {id:1, name:'party_1', code:'party_5'}
    let party6Label = {id:1, name:'party_1', code:'party_6'}
    ...
    const validate = (values) => {
        ...
        if (!values.votingLevel && values.votingLevel !== 0) {
            ...
            errors.voidVotes = 'Void Votes record is required';
            ...
            errors.voidVotes = 'Void votes should not be less than 0'
        }   else if(values.electionType === 0){
            errors.electionType = 'Election type should be selected'
        }   else {
            values.lga=lgaId;
            values.senatorialDistrict = senatorialDistrictId;
            setFormValid(true);
        }
        return errors;
    }
    ...
    const getSenatorialDistricts = () => {
      ...
      apiRequest(`${users}/location`, 'get')
         .then((res) => {
           setLgaId(res.lgaId);
           setSenatorialDistrictId(res.senatorialDistrictId);
           setDistrictname(res.senatorialDistrictName);
                    ...
                    <form onSubmit={handleSubmit} autoComplete="off">
                        ...
                        <div className="mt-4 mb-12">
                            <select
                                ...
                                className="w-full border border-primary rounded-sm py-3 px-2 focus:outline-none bg-transparent placeholder-darkerGray font-medium text-sm text-darkerGray"
                            >
                                <option key="11">{districtname}</option>
                            </select>
                        </div>
                        <div className="mt-4 mb-12">
                            <select
                        ...
                        <div className="flex justify-between items-center w-full">
                            <button type="submit" disabled={(isSubmitting || formFields) && !formValid} className="bg-primary py-4 text-white font-bold rounded-sm focus:outline-none w-4.5/10">
                                {formFields ? 'Updates' : 'Add'} Result
                            </button>
